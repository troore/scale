...
int nv;
  ...
  ht = (hashtab *)malloc(sizeof(hashtab) + (nht-1)*sizeof(hashentry *) + nv*sizeof(hashentry));
  if (!ht)
    return 0;
...
getname(register char *s, int slen)
{
  ...
  if (!(*s++ = Alpha[ch & 0xff])) {
    if (ch != EOF)
      ch = 115;
    return err__fl(f__elist->cierr, ch, "namelist read");
  }
  ...
  if (ch == EOF)
    return err(f__elist->cierr, EOF, "namelist read");
 ...
 top:
  for (;;)
	...
	if (ch == EOF)
	  goto eof;
#else
      return err__fl(a->cierr, 115, where0);
#endif
    }
 have_amp:
  ...
  if (strcmp(buf, nl->name))
    ...
    return err__fl(a->cierr, 118, where0);
#else
  {
    fprintf(stderr, 
  ...
  ht = mk_hashtab(nl);
  if (!ht)
    return err__fl(f__elist->cierr, 113, where0);
  for (;;) {
    for (;;) switch(GETC(ch)) {
    case EOF:
      if (got1)
  ...
  havename:
    v = hash(ht, buf);
    if (!v)
      return err__fl(a->cierr, 119, where);
    ...
    if (ch == '(' /*)*/ ) {
      ...
      if (!(dims = v->dims)) {
	if (type != TYCHAR)
	  return err__fl(a->cierr, 122, where);
	if (k = getdimen(&ch, dn, (ftnlen)size, (ftnlen)size, &b))
	  return err__fl(a->cierr, k, where);
	if (ch != ')')
	  return err__fl(a->cierr, 115, where);
	b1 = dn->extent;
	if ((--b < 0) || (b + b1 > size))
	  return 124;
	iva += b;
	...
	return err__fl(a->cierr, 115, where);
      ...
      b -= b0;
      if ((b < 0) || (b >= nomax))
	return err__fl(a->cierr, 125, where);
      iva += size * b;
      dims = dims1;
...
s_rsne(cilist *a)
{
  ...
  if (f__curunit->uwrt && f__nowreading(f__curunit))
    return err(a->cierr, errno, where0);
