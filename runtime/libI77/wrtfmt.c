		...
		/* instead we know too much about stdio */
{
  ...
  if (f__external == 0) {
	...
	f__hiwater = f__recpos;
      ...
      f__icptr += cursor;
      if (f__recpos < 0)
	return err(f__elist->cierr, 110, "left off");
    } else if (cursor > 0) {
      if (f__recpos + cursor >= f__svic->icirlen)
	return err(f__elist->cierr, 110, "recend");
      if (f__hiwater <= f__recpos)
	for (; cursor > 0; cursor--)
	  (*f__putn)(' ');
...
/* 	(void) fseek(f__cf, (long)cursor, SEEK_CUR); */
/*       else */
	return err(f__elist->cierr, 106, "fmt");
    if (f__hiwater < f__recpos)
      f__hiwater = f__recpos;
    f__recpos += cursor;
  }
...
w_ed(struct syl *p, char *ptr, ftnlen len)
{
  ...
  case D:
    return wrt_D((ufloat *)ptr, p->p1, p->p2, p->p3, len);
  case E:
  case EE:
  ...
  case F:
    return(wrt_F((ufloat *)ptr, p->p1, p->p2, len));
  case Z: /* Z and ZM assume 8-bit bytes. */
