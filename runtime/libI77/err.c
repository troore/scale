...
int (*f__getn)(void);
int (*f__putn)(int);	/* for formatted io */
...
int f__cursor;	        /* place in current record */
int f__hiwater;	        /* place in current record */
int f__scale;
char *f__icptr;
...
f__nowwriting(unit *x)
{
  ...
  if (x->uwrt == 3) { /* just did write, rewind */
    ...
    loc=ftell(x->ufd);
    if ((fclose(x->ufd) < 0) ||
	((k = x->uwrt == 2 ? creat(x->ufnm, 0666) : open(x->ufnm, O_WRONLY)) < 0) ||
